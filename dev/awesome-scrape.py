...
def main():
    ...
    for key, value in merged.items():
        for title in value['titles']:
            ...
            existing_lib = next((otherlib for otherlib in bytitles[normalized_title] if have_same_name(otherlib, lib) or have_common_url(otherlib, lib)), None)
            if existing_lib:
                print(f"lib already exists under {normalized_title}:\nlib:\n\t{lib}\nexisting:\n\t{existing_lib}")
            else:
                bytitles[normalized_title].append(lib)
    
    bytitles_str = ""
    for title, libs in sorted(bytitles.items()):
        bytitles_str += f"\n\n## {title}\n\n"
        for lib in sorted(libs, key=lambda k: k['name']):
            bytitles_str += f"- [{lib['name']}]({next(iter(lib['urls']))}) - {lib['description']}\n"
    
    with open('./dev/merged-awesome-django.md', mode='w') as f:
        f.write(bytitles_str)
